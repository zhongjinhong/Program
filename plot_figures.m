function [  ] = plot_figures( experiment_num )
    ...
    if experiment_num ~= 10
        ...
        hold on;
        linewidth=2;
        x_label=(begin_num:end_num)*10;
        if experiment_num == 3         
            x_label=(1:size(acc_LFC,2))*5;
        end
        ...
        han(4,1)=plot(acc_YAN,'-ys','LineWidth',linewidth,'MarkerFaceColor','y');
        han(5,1)=plot(x_label, acc_M3V,'-ks','LineWidth',linewidth,'MarkerFaceColor','k');
        han(6,1)=plot(x_label, acc_Soft_LCM,'-rs','LineWidth',linewidth,'MarkerFaceColor','r');
        title(title_name,'FontSize',16)
        xlabel('The instance number','FontSize',16);
        ylabel('The test accuracy(%)','FontSize',16);
        legend(han(1:6),'Raykar''s Model','Kajino''s Model','Majority Voting','YAN''s Model','M3V','Soft-LCM');
%         if experiment_num ~= 3  
%             xlim([100 200])
%         end
...
%         han(6,1)=plot(auc_DCS_LA,'-ys','LineWidth',linewidth,'MarkerFaceColor','y'); 
        ...
        han(5,1)=plot(x_label, auc_M3V,'-ks','LineWidth',linewidth,'MarkerFaceColor','k');
        han(6,1)=plot(x_label, auc_Soft_LCM,'-rs','LineWidth',linewidth,'MarkerFaceColor','r');
        title(title_name,'FontSize',16)
        xlabel('The instance number','FontSize',16);
...
%         end
        ...
        han(5,1)=plot(x_label,Time_M3V1,'-ks','LineWidth',linewidth,'MarkerFaceColor','k');
        han(6,1)=plot(x_label,Time_Soft_LCM,'-rs','LineWidth',linewidth,'MarkerFaceColor','r'); 
