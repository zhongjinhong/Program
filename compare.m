    ...
    for num=begin_num:end_num
        for repeat_num=1:total_repeat_num
            switch experiment_num
                case {1,2,3,4,5,6,7,8,9,21,22,23,25,26,27}
                    file_name=sprintf('%s%s%d%s%d%s',input_file_dir,'X_',num*5*2,'_',repeat_num,'.txt');
                    X=load(file_name);  
                    file_name=sprintf('%s%s%d%s%d%s',input_file_dir,'Y_',num*5*2,'_',repeat_num,'.txt');
                    Y=load(file_name);
%                 case {}
%                     file_name=sprintf('%s%s',input_file_dir,'X.txt');
%                     X=load(file_name);  
%                     file_name=sprintf('%s%s%d%s',input_file_dir,'Y_',repeat_num,'.txt');
%                     Y=load(file_name);   
                case {11,24,28}
                    file_name=sprintf('%s%s',input_file_dir,'X.txt');
                    X=load(file_name);  
                    file_name=sprintf('%s%s%s',input_file_dir,'Y_1','.txt');
                    Y_temp=load(file_name);
                    Y = Y_temp;
                    for i = 1:num-1
                        Y = [Y Y_temp];
                    end
                case {10}
                    file_name=sprintf('%s%s',input_file_dir,'X.txt');
                    X=load(file_name);  
                    file_name=sprintf('%s%s%d%s%d%s',input_file_dir,'Y_',num*5,'_',repeat_num,'.txt');
                    Y=load(file_name);                    
            end
            n=size(X,1);    
            expert_num=size(Y,2);
